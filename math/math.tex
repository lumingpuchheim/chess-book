% This file now focuses on the book-specific content and uses shared
% preprocess/postprocess files from the project root.

% Book-specific metadata (overrides defaults in preprocess.tex)
\def\BookTitle{My Math Notes}
\def\BookAuthor{Ming Lu} % or another author if you like

% Index page offset: controls how much we shift page numbers in \hyperpage.
% This is used in postprocess.tex; adjust here per-book as needed.
\def\IndexPageOffset{0}


\input{../preprocess}

%----------------------------------------------------------------------------------------
%	Main Content
%----------------------------------------------------------------------------------------
\chapter*{Preface}
\thispagestyle{plain} % Ensure plain pagestyle is applied (with page numbers in footer)
\addcontentsline{toc}{chapter}{Preface}
\input{preface}
\input{motivation/why-i-should-learn-math}
\input{fun/fitch-cheney-trick}
\input{fun/rock-paper-scissors}
\input{fun/15-puzzle}
\input{fun/tic-tac-toe}
\input{fun/magic-square}
\input{proof/pythagorean-theorem}
\input{proof/reductio_ad_absurdum}
\input{proof/harmonic_series}
\input{proof/cayley_theorem}
\input{principles/inclusion-exclusion}
\input{topics/eulers-totient-function}
\input{topics/rsa}
\input{topics/rsa-security}
\input{topics/linear_space_and_fourier_transformation}

\input{random_variable}
\input{common_distributions}
\input{central_limit_theorem}
\input{birthday_paradox}
\input{powerspin/main}



\def\IndexPageOffset{1}
\input{../postprocess}
