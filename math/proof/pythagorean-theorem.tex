...
\hline
Example & \(a\) & \(b\) & \(c\) & \(a^2\) & \(b^2\) & \(a^2+b^2\) & \(c^2\) \\
\hline
1 & 3 & 4 & 5 & 9 & 16 & 25 & 25 \\
\hline
2 & 5 & 12 & 13 & 25 & 144 & 169 & 169 \\
\hline
3 & 8 & 15 & 17 & 64 & 225 & 289 & 289 \\
\hline
4 & 7 & 24 & 25 & 49 & 576 & 625 & 625 \\
\hline
\end{tabular}
\end{center}
...
\section{Second Proof: Using Similar Triangles}
This proof is another classic one (also listed on the Wikipedia page): draw an
altitude from the right-angle corner to the hypotenuse, then use triangle
similarity.
...
\begin{tikzpicture}[scale=0.95]
  ...
  \draw (H) ++(0.22,0) -- ++(0,0.22) -- ++(-0.22,0);

  % side labels
  \node[left] at (0.95,1.6) {\(b=AC\)};
  \node[above right] at (4.6,1.9) {\(a=BC\)};

  % segment labels on hypotenuse split
  \node[below] at (1,0) {\(AH\)};
  \node[below] at (4.6,0) {\(HB\)};
...
In this picture:
\begin{itemize}
  \item \(\triangle ABC\) is right-angled at \(C\);
  \item \(CH\) is perpendicular to \(AB\);
  \item the big triangle and the two smaller triangles are similar.
\end{itemize}

From similarity, we get these two key relations:
\[
BC^2=AB\cdot HB,\qquad AC^2=AB\cdot AH.
\]
Now add them:
\[
BC^2+AC^2=AB\cdot HB+AB\cdot AH
=AB\,(HB+AH).
\]
But \(HB+AH=AB\), so
\[
BC^2+AC^2=AB^2.
\]
Rename \(BC=a\), \(AC=b\), and \(AB=c\):
\[
a^2+b^2=c^2.
\]

So the theorem is proved again, this time by similarity instead of rearranging
areas.
