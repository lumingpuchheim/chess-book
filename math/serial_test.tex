...
\begin{enumerate}
  ...
  \item \textbf{Count observed triples.}\\
        For each draw, you read the three wheel outputs \emph{horizontally}
        across the row, forming one 3-tuple:
        \[
          (w_1,w_2,w_3).
        \]
        For example, if a row contains \((7,\texttt{symbol},14)\), that is one
        observed triple.  You then increment the corresponding cell in a
        3-dimensional count table \(\texttt{observed}[i,j,k]\).

        Repeating this for every draw row produces:
        \begin{itemize}
